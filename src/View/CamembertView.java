...
public class CamembertView extends JComponent implements MouseListener,
    ...
    public void buildGraphics() {
        ...
        // create central arcs
        emptyCenter = new Arc2D.Double(pieCenter.getX()
                - (pieSize.width / 2 - pieThickness), pieCenter.getY()
                - (pieSize.height / 2 - pieThickness), pieSize.width - 2
                * pieThickness, pieSize.width - 2 * pieThickness, 0, 360,
                Arc2D.PIE);

        center = new Arc2D.Double(pieCenter.getX() - pieCenterSize.width / 2,
                pieCenter.getY() - pieCenterSize.height / 2,
                pieCenterSize.width, pieCenterSize.height, 0, 360, Arc2D.PIE);
    ...
    public void paintComponent(Graphics g) {
        ...
        GradientPaint gp = new GradientPaint(0, 110, Color.WHITE, 0, 150,
                Color.BLUE);
        g2d.setPaint(gp);
        ...
        if (!controller.isSelected()) {
            ...
            for (int i = 0; i < model.size(); i++) {
                ...
                double x = pieCenter.getX()
                        + positionXOnCircle(pieSize.getWidth() / 2 + tagOffset,
                        midangle);
                double y = pieCenter.getY()
                        - positionYOnCircle(
                        pieSize.getHeight() / 2 + tagOffset, midangle);

                g2d.setColor(new Color(100, 100, (100 + 20 * i) % 255));
                if (y < pieCenter.getY()) {
                    if (x > pieCenter.getX()) { // top right
                        Rectangle2D tag = new Rectangle2D.Double(x, y
                                - tagSizeNotSelected.height,
                                tagSizeNotSelected.width,
                                tagSizeNotSelected.height);
                        g2d.fill(tag);
                        ...
                        Rectangle2D tagUnderline = new Rectangle2D.Double(x, y,
                        ...
                        Rectangle2D tag = new Rectangle2D.Double(x
                                - tagSizeNotSelected.width, y
                                - tagSizeNotSelected.height,
                                tagSizeNotSelected.width,
                                tagSizeNotSelected.height);
                        g2d.fill(tag);
                        ...
                        g2d.fill(stem);
                        g2d.setColor(Color.black);
                        Rectangle2D tagUnderline = new Rectangle2D.Double(x
                                - tagSizeNotSelected.width, y,
                                tagSizeNotSelected.width, 3);
                        g2d.fill(tagUnderline);
                        g2d.setColor(Color.WHITE);
        ...
        if (controller.isSelected()) {
            ...
            for (int i = 0; i < model.size(); i++) {
                ...
                if (i == controller.getSelectedPie()) {
                    ...
                    double y = pieCenter.getX()
                            - positionYOnCircle(160, midangle);
                    ...
                    if (y < pieCenter.getY()) {
                        if (x > pieCenter.getX()) { // top right
                            ...
                            g2d.fill(stem);
                            g2d.setColor(Color.black);
                            Rectangle2D tagUnderline = new Rectangle2D.Double(
                            ...
                            while (measurer.getPosition() < characterIterator
                                    .getEndIndex()) {
                                TextLayout textLayout = measurer
                                        .nextLayout(tagSizeSelected.width - 15);
